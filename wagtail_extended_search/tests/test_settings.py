...
from wagtail_extended_search.index import BaseField, SearchField
...
from wagtail_extended_search.settings import (
    ...
    settings_singleton,
    wagtail_extended_search_settings,
)
...
class TestNestedChainMap:
    def test_init(self, mocker):
        mock_parent = mocker.patch(
            "wagtail_extended_search.settings.ChainMap.__init__", return_value=None
        )
    ...
    def test_getitem(self, mocker):
        mock_missing = mocker.patch(
            "wagtail_extended_search.settings.NestedChainMap.__missing__",
        ...
        mock_prefix = mocker.patch(
            "wagtail_extended_search.settings.NestedChainMap._get_prefixed_key_name",
    ...
    def test_missing(self, mocker):
        mocker.patch(
            "wagtail_extended_search.settings.NestedChainMap._get_all_prefixed_keys_from_nested_maps",
            return_value=["a", "b"],
        )
        mock_getitem = mocker.patch(
            "wagtail_extended_search.settings.NestedChainMap._getitem_from_nested_maps_for_prefixed_key",
            return_value="--foobar--",
        )
        map1 = {"foo": "a", "bar": "b"}
        map2 = {"foo": 2, "foobar": "c"}
...
class TestSearchSettings:
    ...
    def test_get_all_indexed_fields(self, mocker):
        mock_get_models = mocker.patch(
            "wagtail_extended_search.settings.get_indexed_models"
        )
        instance = SearchSettings()
    ...
    def test_initialise_field_dict(self, mocker):
        mocker.patch(
            "wagtail_extended_search.settings.get_settings_field_key",
            return_value="--settings-field-key--",
        )
        mock_get_fields = mocker.patch(
            "wagtail_extended_search.settings.SearchSettings._get_all_indexed_fields"
        )
    ...
    def test_initialise_env_dict(self, mocker):
        mock_env = mocker.patch("wagtail_extended_search.settings.env")
        mocker.patch(
            "wagtail_extended_search.settings.NestedChainMap._get_all_prefixed_keys_from_nested_maps",
            return_value=[],
        )  # for all_keys
    ...
    def test_singleton(self):
        ...
        from wagtail_extended_search.settings import settings_singleton as second_import
    ...
    def test_to_dict(self):
        ...
        assert isinstance(wagtail_extended_search_settings, dict)
    ...
    def test_updated_settings_affect_exported_dict(self):
        from wagtail_extended_search import settings  # 1
        from wagtail_extended_search.settings import (
            wagtail_extended_search_settings,
        )  # 2

        original_settings = copy.deepcopy(settings.wagtail_extended_search_settings)
        assert original_settings == settings.wagtail_extended_search_settings  # 1
        assert original_settings == wagtail_extended_search_settings  # 2
        ...
        settings.wagtail_extended_search_settings = settings_singleton.to_dict()

        # exported module dict changed
        assert (
            original_settings != settings.wagtail_extended_search_settings
        )  # <-- in general use this style import
        # ... but exported dict still not changed yet :'(
        assert original_settings == wagtail_extended_search_settings
        ...
        assert settings_singleton.to_dict() == settings.wagtail_extended_search_settings
