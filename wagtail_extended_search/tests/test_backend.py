...
class TestExtendedSearchQueryCompiler:
    def test_remap_fields_works_the_same_as_parent_init(self):
        ...
        compiler = ExtendedSearchQueryCompiler(Page.objects.all(), query)
        es7_compiler = Elasticsearch7SearchQueryCompiler(Page.objects.all(), query)
    ...
    def test_remap_fields_handles_parent_relations(self, mocker):
        ...
        mock_get_column_name = mocker.patch(
            "wagtail_extended_search.backends.backend.Elasticsearch7Mapping.get_field_column_name",
    ...
    def test_join_compile_queries_output_format_and_uses_compile_query(self, mocker):
        mock_compile = mocker.patch(
            "wagtail_extended_search.backends.backend.ExtendedSearchQueryCompiler._compile_query",
            return_value="--FOO--",
        )
        query = PlainText("quid")
        compiler = ExtendedSearchQueryCompiler(Page.objects.all(), query)
    ...
    def test_get_inner_query_works_the_same_as_parent(self, mocker):
        query = PlainText("foo")
        compiler = ExtendedSearchQueryCompiler(Page.objects.all(), query)
        compiler_es7 = Elasticsearch7SearchQueryCompiler(Page.objects.all(), query)
        compiler_es7.mapping.all_field_name = None
        mock_compile_plaintext = mocker.patch(
            "wagtail_extended_search.backends.backend.ExtendedSearchQueryCompiler._compile_plaintext_query",
        )
        ...
        mock_compile_phrase = mocker.patch(
            "wagtail_extended_search.backends.backend.ExtendedSearchQueryCompiler._compile_phrase_query",
        )
        ...
        mock_compile_fuzzy = mocker.patch(
            "wagtail_extended_search.backends.backend.ExtendedSearchQueryCompiler._compile_fuzzy_query",
        )
        ...
        mock_join_and_compile = mocker.patch(
            "wagtail_extended_search.backends.backend.ExtendedSearchQueryCompiler._join_and_compile_queries",
        )
...
class TestOnlyFieldSearchQueryCompiler:
    def test_compile_query_uses_parent_when_not_onlyfields(self, mocker):
        ...
        query = PlainText("quid")
        compiler = OnlyFieldSearchQueryCompiler(Page.objects.all(), query)
        field = Field("bar")
        compiler._compile_query(query, field, 3.5)
        mock_parent.assert_called_once_with(query, field, 3.5)

        mock_parent.reset_mock()
        query = OnlyFields(PlainText("quid"), fields=["foo"], only_model=Page)
        compiler = OnlyFieldSearchQueryCompiler(Page.objects.all(), query)
    ...
    def test_compile_query_uses_remap_fields(self, mocker):
        mock_remap = mocker.patch(
            "wagtail_extended_search.backends.backend.ExtendedSearchQueryCompiler._remap_fields"
        )
        query = OnlyFields(
            PlainText("quid"),
            fields=["foo"],
        ...
        compiler._compile_query(query, Field("bar"), 3.5)
        assert (
            call(["foo"], get_searchable_fields__kwargs={"only_model": Page})
    ...
    def test_compile_query_onlyfields_logic(self, mocker):
        ...
        query = OnlyFields(subquery, fields=["foo"], only_model=Page)
        compiler = OnlyFieldSearchQueryCompiler(Page.objects.all(), query)
...
class TestBoostSearchQueryCompiler:
    def test_compile_query_catches_all(self, mocker):
        mock_compile_fuzzy = mocker.patch(
            "wagtail_extended_search.backends.backend.BoostSearchQueryCompiler._compile_fuzzy_query"
        )
        mock_compile_phrase = mocker.patch(
            "wagtail_extended_search.backends.backend.BoostSearchQueryCompiler._compile_phrase_query"
        )
        query = PlainText("quid")
        field = Field("baz")
        compiler = BoostSearchQueryCompiler(Page.objects.all(), query)
...
class TestNestedSearchQueryCompiler:
    ...
    def test_compile_query_catches_nested(self, mocker):
        mock_compile_nested = mocker.patch(
            "wagtail_extended_search.backends.backend.NestedSearchQueryCompiler._compile_nested_query"
        )
        query = PlainText("quid")
        field = Field("baz")
        compiler = NestedSearchQueryCompiler(Page.objects.all(), query)
...
class TestFilteredSearchQueryCompiler:
    def test_compile_query_catches_filtered(self, mocker):
        mock_compile_filtered = mocker.patch(
            "wagtail_extended_search.backends.backend.FilteredSearchQueryCompiler._compile_filtered_query"
        )
        query = PlainText("quid")
        field = Field("baz")
    ...
    def test_compile_filtered_query(self, mocker):
        mock_process_lookup = mocker.patch(
            "wagtail_extended_search.backends.backend.FilteredSearchQueryCompiler._process_lookup",
            return_value="foobar",
        )
        query = Filtered(
            Phrase("quid"),
    ...
    def test_process_lookup(self, mocker):
        mock_parent = mocker.patch(
            "wagtail_extended_search.backends.backend.ExtendedSearchQueryCompiler._process_lookup"
        )
        mocker.patch(
            "wagtail_extended_search.backends.backend.Elasticsearch7Mapping.get_field_column_name",
            return_value="foobar",
...
class TestFilteredSearchMapping:
    def test_get_field_column_name(self, mocker):
        ...
        map = FilteredSearchMapping(model=Page)
        map.get_field_column_name("foo")
        mock_parent.assert_called_once()

        mock_parent.reset_mock()
        map.get_field_column_name(Page._meta.fields[0])
        mock_parent.assert_called_once()
