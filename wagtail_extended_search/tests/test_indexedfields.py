...
class TestModelFieldNameMixin:
    ...
    def test_get_definition_model(self, mocker):
        ...
        mock_parent_get_definition_model = mocker.patch(
            "wagtail_extended_search.tests.testing_classes.Base.get_definition_model",
            return_value=None,
        )
        mock_get_base_model_field_name = mocker.patch(
            "wagtail_extended_search.index.ModelFieldNameMixin.get_base_model_field_name",
            return_value="foobar",
        )
    ...
    def test_get_value(self, mocker):
        mock_parent_get_value = mocker.patch(
            "wagtail_extended_search.tests.testing_classes.Base.get_value",
            return_value="foo",
        )
    ...
    def test_get_base_model_field_name(self, mocker):
        mock_get_base_field = mocker.patch(
            "wagtail_extended_search.index.ModelFieldNameMixin.get_base_field",
...
class TestRelatedFields:
    ...
    def test_get_select_on_queryset(self, mocker):
        ...
        mock_get_field = mocker.patch(
            "wagtail_extended_search.index.RelatedFields.get_field"
        )
    ...
    def test_generate_fields(self, mocker):
        ...
        mock_relation = mocker.patch(
            "wagtail_extended_search.index.RelatedFields.is_relation_of"
        )
        field = RelatedFields("foo", [], model_field_name="bar")
        assert field.fields == []
...
class TestIndexedField:
    ...
    def test_generate_fields(self, mocker):
        mock_generate_search = mocker.patch(
            "wagtail_extended_search.index.IndexedField.generate_search_fields",
            return_value=["--search-field--", "--search-field-2--"],
        )
        mock_generate_autocomplete = mocker.patch(
            "wagtail_extended_search.index.IndexedField.generate_autocomplete_fields",
            return_value=["--autocomplete-field--"],
        )
        mock_generate_filter = mocker.patch(
            "wagtail_extended_search.index.IndexedField.generate_filter_fields",
            return_value=["--filter-field--", "--filter-field-2--"],
        )
        mock_relation = mocker.patch(
            "wagtail_extended_search.index.IndexedField.is_relation_of"
        )
        model = mocker.Mock()
    ...
    def test_generate_search_fields(self, mocker):
        mock_get_variants = mocker.patch(
            "wagtail_extended_search.index.IndexedField.get_search_field_variants",
            return_value=[],
        )
        mock_field = mocker.patch("wagtail_extended_search.index.SearchField")
        model = mocker.Mock()
        field = IndexedField("foo", model_field_name="bar")
    ...
    def test_generate_autocomplete_fields(self, mocker):
        mock_get_variants = mocker.patch(
            "wagtail_extended_search.index.IndexedField.get_autocomplete_field_variants",
            return_value=[],
        )
        mock_field = mocker.patch("wagtail_extended_search.index.AutocompleteField")
        model = mocker.Mock()
        field = IndexedField("foo", model_field_name="bar")
    ...
    def test_generate_filter_fields(self, mocker):
        mock_get_variants = mocker.patch(
            "wagtail_extended_search.index.IndexedField.get_filter_field_variants",
            return_value=[],
        )
        mock_field = mocker.patch("wagtail_extended_search.index.FilterField")
        model = mocker.Mock()
        field = IndexedField("foo", model_field_name="bar")
...
class TestMultiQueryIndexedField:
    ...
    def test_get_search_field_variants(self, mocker):
        mock_get_name = mocker.patch(
            "wagtail_extended_search.index.get_indexed_field_name",
        ...
        assert field.get_search_field_variants(model) == [
            (
                ...
                {
                    "es_extra": {
                        "analyzer": settings.wagtail_extended_search_settings[
                            "analyzers"
                        ][AnalysisType.TOKENIZED.value]["es_analyzer"]
                    }
                },
            )
        ]
            ...
            (
                ("--field-name--",),
                {
                    "es_extra": {
                        "analyzer": settings.wagtail_extended_search_settings[
                            "analyzers"
                        ][AnalysisType.EXPLICIT.value]["es_analyzer"]
                    }
                },
            ),
            (
