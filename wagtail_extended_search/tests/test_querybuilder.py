...
from wagtail_extended_search.index import (IndexedField, RelatedFields,
                                           SearchField)
from wagtail_extended_search.query import Filtered, Nested, OnlyFields
...
class TestCustomQueryBuilder:
    ...
    def test_build_query_for_model(self, mocker):
        mock_combine = mocker.patch(
            "wagtail_extended_search.query_builder.CustomQueryBuilder._combine_queries",
            return_value="foo",
        )
        mock_build_search_query = mocker.patch(
            "wagtail_extended_search.query_builder.CustomQueryBuilder._build_search_query",
            return_value=None,
        )
    ...
    def test_get_extended_models_with_unique_indexed_fields(self, mocker):
        ...
        mock_get_models = mocker.patch(
            "wagtail_extended_search.query_builder.get_indexed_models",
            return_value=[base_model_class],
        )
        mock_mro = mocker.patch(
            "wagtail_extended_search.query_builder.inspect.getmro",
    ...
    def test_get_search_query(self, mocker):
        ...
        mock_build_search_query = mocker.patch(
            "wagtail_extended_search.query_builder.CustomQueryBuilder.build_search_query",
            return_value=built_query,
        )
        output_query = PlainText("bar")
        mock_swap_variables = mocker.patch(
            "wagtail_extended_search.query_builder.CustomQueryBuilder.swap_variables",
    ...
    def test_build_search_query(self, mocker):
        ...
        mock_get_extended_models = mocker.patch(
            "wagtail_extended_search.query_builder.CustomQueryBuilder.get_extended_models_with_unique_indexed_fields",
            return_value=[extended_model_class],
        )
        mock_get_query = mocker.patch(
            "wagtail_extended_search.query_builder.CustomQueryBuilder.build_query_for_model",
            ...
            Or(
                [
                    Filtered(
                        ...
                        filters=[
                            ("content_type_id", "notin", ["mock.extended_model"]),
                        ],
                    ),
                    Filtered(
                        subquery=PlainText("foo"),
                        filters=[
                            ("content_type_id", "in", ["mock.extended_model"]),
                        ],
                    ),
                ]
            )
...
class TestQueryBuilder:
    ...
    def test_get_boost_for_field_querytype_analysistype(self, mocker):
        mock_boost_qt = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder._get_boost_for_querytype",
            return_value=25,
        )
        mock_boost_at = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder._get_boost_for_analysistype",
            return_value=125,
        )
    ...
    def test_get_boost_for_field(self, mocker):
        mock_get_key = mocker.patch(
            "wagtail_extended_search.query_builder.search_settings.get_settings_field_key",
        ...
        settings.wagtail_extended_search_settings["boost_parts"]["fields"][
            "--settings-key--"
        ] = 333.33
    ...
    def test_get_boost_for_analysistype(self):
        ...
        settings.wagtail_extended_search_settings["boost_parts"]["analyzers"][
            "explicit"
        ] = 888.88
        assert (
    ...
    def test_get_boost_for_querytype(self):
        ...
        settings.wagtail_extended_search_settings["boost_parts"]["query_types"][
            "phrase"
        ] = 888.88
        assert (
    ...
    def test__get_search_query(self, mocker):
        ...
        mock_build_query_indexfield = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder._build_search_query_for_indexfield",
            return_value="index-query",
        )
        mock_build_query_searchfield = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder._build_search_query_for_searchfield",
            return_value="search-query",
        )
        mock_combine_queries = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder._combine_queries",
            return_value=query,
        )
        mock_infer_analyzer = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder.infer_analyzer_from_field"
        )
    ...
    def test_infer_analyzer_from_field(self, mocker):
        ...
        analyzer_settings = settings.wagtail_extended_search_settings["analyzers"]
    ...
    def test_build_search_query_for_indexfield(self, mocker):
        ...
        mock_build_searchquery = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder._build_searchquery_for_query_field_querytype_analysistype",
    ...
    def test_build_search_query_for_searchfield(self, mocker):
        ...
        assert (
            len(
                settings.wagtail_extended_search_settings["analyzers"][
        ...
        expected = []
        for q in settings.wagtail_extended_search_settings["analyzers"][
            AnalysisType.TOKENIZED.value
        ]["query_types"]:
            expected.append(
                call(
    ...
    def test_build_searchquery_for_query_field_querytype_analysistype(self, mocker):
        mock_boost = mocker.patch(
            "wagtail_extended_search.query_builder.QueryBuilder._get_boost_for_field_querytype_analysistype"
        )
        mocker.patch(
            "wagtail_extended_search.query_builder.get_indexed_field_name",
            return_value="foobar",
        )
        model = mocker.Mock()
        field = mocker.Mock()
